...
export const actions = {
   ...
   setCurrentPage: (newCurrentPage: number) => ({ type: 'SET_CURRENT_PAGE', currentPage: newCurrentPage } as const),
   setUsersTotalCount: (totalCount: number) => ({ type: 'SET_USERS_TOTAL_COUNT', count: totalCount } as const),
   setIsFetching: (isFetching: boolean) => ({ type: 'TOGGLE_IS_FETCHING', isFetching } as const),
   toggleFollowProgress: (isFetching: boolean, userId: number) => ({ type: 'TOGGLE_IS_FOLLOWING_PROGRESS', isFetching, userId } as const),
}
...
const followUnfollowFlow = async (dispatch: Dispatch<ActionTypes>, userId: number, apiMethod: any, actionCreator: any) => {
